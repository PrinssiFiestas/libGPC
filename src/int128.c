...
GPUInt128 gp_uint128_long_mul64(uint64_t a, uint64_t b)
{
    ...
    // and implementation complexity. Or is it? Small todo: benchmark on 32-bit ARM.
...
{
    gp_assume(gp_uint128_not_equal(b, gp_uint128(0, 0)), "Division by zero.");
...
// FIXME some of the assertions are a bit harsh. LLVM saturates, which is what
// user may expect.
...
GPUInt128 gp_uint128_convert_f64(double a)
{
    ...
    #endif
    gp_assume(a >= 0., "Negative value cannot be represented as an unsigned type.");
...
GPUInt128 gp_uint128_convert_f32(float a)
{
    ...
    gp_assume( ! isinf(a), "Inf cannot be represented as an integral type.");
    #endif
    gp_assume(a >= 0.f, "Negative value cannot be represented as an unsigned type.");
