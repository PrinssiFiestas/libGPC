...
// laundering is good enough, others need a wrapper.

#if defined(__x86_64__) || UINTPTR_MAX == UINT_MAX // no need to wrap threads

int gp_thread_create(gp_thread_t* thr, int(*func)(void*), void *arg)
{
    int res = pthread_create(thr, 0, gp_launder(func), arg);

    if(res == 0)
        return GP_THREAD_SUCCESS;
    return res == ENOMEM ? GP_THREAD_NOMEM : GP_THREAD_ERROR;
}
...
// Thread local storage allows allocating wrapper on the heap (for lifetime),
// which can be automatically deallocated on thread exit.s
static pthread_key_t gp_s_thread_wrapper_key;
static pthread_once_t gp_s_thread_wrapper_once = GP_ONCE_FLAG_INIT;

static void* gp_s_thread_wrapper_routine(void*_wrapper)
{
...
void* gp_s_thread_wrapper_arg(int(*f)(void*), void* arg)
{
...
int gp_thread_create(gp_thread_t* thr, int(*func)(void*), void *arg)
{
    void* wrapper_arg = gp_s_thread_wrapper_arg(func, arg);
    if (wrapper_arg == NULL)
        return errno == ENOMEM ? GP_THREAD_NOMEM : GP_THREAD_ERROR;
    int res = pthread_create(
        thr, 0,
        gp_s_thread_wrapper_routine,
        wrapper_arg);
...
#define GP_NO_TIMED_MUTEX 1
#endif

#ifdef GP_NO_TIMED_MUTEX
#define GP_TIMEDLOCK_POLL_INTERVAL 5000000
#endif

int mtx_timedlock(GPMutex *mtx, double t)
{
    struct timespec ts = gp_timespec_from_time(t);
